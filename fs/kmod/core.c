...
static int ds_acquire_block(struct super_block *sb) {
...
#define ds_ACQUIRE_BLOCK(sb, blockptr, create, new, dirty, ptr) (create) ? ds_get_or_allocate_block((sb), (blockptr), new, (void(*)(void*))(dirty), (ptr)) : *(blockptr)

// If *block isn't currently acquired (is == 0), then read it from the disk and
// make *block point to it.
static int ds_get_or_allocate_block(struct super_block *sb, int *block, int *new_out, void (*mark_dirty)(void*), void *dirty_data) {
    int res;

    if (new_out)
        *new_out = 0;
    if (*block == 0) {
        if ((res = ds_acquire_block(sb)) < 0)
            return res;
        *block = res;
        if (new_out)
            *new_out = 1;
...
int ds_get_block(struct inode *inode, sector_t iblock, struct buffer_head *bh, int create)
{
    struct ftfs_inode_info *ds_inode = (struct ftfs_inode_info*)inode->i_private;
    int block = 0;
    int new = 0;
    ...
    if (iblock < 12)
    {
        ...
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, &ds_inode->blocks[iblock], create, &new, mark_inode_dirty, inode)) <= 0)
            return block;
    ...
    if (iblock < 0x100)
    {
        ...
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, &ds_inode->blocks[12], create, NULL, mark_inode_dirty, inode)) <= 0)
            return block;
        LOG("       Lvl0 block[%d]", block);
        ibh = sb_bread(inode->i_sb, block);
        if (!ibh)
            return -ENOMEM;
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, &((__le32*)ibh->b_data)[iblock], create, &new, mark_buffer_dirty, ibh)) <= 0)
            return block;
    ...
    if (iblock < 0x10000)
    {
        ...
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, &ds_inode->blocks[13], create, NULL, mark_inode_dirty, inode)) <= 0)
            return block;
        LOG("       Lvl0 block[%d]", block);
        ibh = sb_bread(inode->i_sb, block);
        if (!ibh)
        ...
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, i1, create, NULL, mark_buffer_dirty, ibh)) <= 0)
            return block;
        LOG("       Lvl1 block[%d]", block);
        brelse(ibh);
        ibh = sb_bread(inode->i_sb, block);
        ...
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, i2, create, &new, mark_buffer_dirty, ibh)) <= 0)
            return block;
    ...
    if (iblock < 0x1000000)
    {
        ...
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, &ds_inode->blocks[14], create, NULL, mark_inode_dirty, inode)) <= 0)
            return block;
        LOG("       Lvl0 block[%d]", block);
        ibh = sb_bread(inode->i_sb, block);
        if (!ibh)
        ...
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, i2, create, NULL, mark_buffer_dirty, ibh)) <= 0)
            return block;
        LOG("       Lvl2 block[%d]", block);
        brelse(ibh);
        ibh = sb_bread(inode->i_sb, block);
        ...
        if ((block = ds_ACQUIRE_BLOCK(inode->i_sb, i3, create, &new, mark_buffer_dirty, ibh)) <= 0)
            return block;
...
static int ds_readpage(struct file *file, struct page *page)
{
    return mpage_readpage(page, ds_get_block);
}

static int ds_writepage(struct page *page, struct writeback_control *wbc)
{
    return block_write_full_page(page, ds_get_block, wbc);
}

static int ds_write_begin(struct file *file, struct address_space *mapping,
        loff_t pos, unsigned len, unsigned flags, struct page **pagep,
        void **fsdata)
{
    return block_write_begin(mapping, pos, len, flags, pagep, ds_get_block);
}
...
// pagecache.
const struct address_space_operations ds_aops = {
    .readpage       = ds_readpage,
    .writepage      = ds_writepage,
    .write_begin    = ds_write_begin,
...
const struct inode_operations ds_file_inode_operations = {
    .setattr        = simple_setattr,
    .getattr        = simple_getattr,
};

const struct inode_operations ds_special_inode_operations = {
    .setattr        = simple_setattr,
    .getattr        = simple_getattr,
};

const struct file_operations ds_file_operations = {
...
extern const struct super_operations ds_ops;

static int ds_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	struct super_block *sb = dentry->d_sb;
	struct ftfs_fs_info *fsinfo = (struct ftfs_fs_info*)sb->s_fs_info;
	struct ftfs_super_block *ds_sb = fsinfo->super_block;

	buf->f_type    =    sb->s_magic;              /* magic */
	buf->f_bsize   =    sb->s_blocksize;          /* optimal transfer block size */
	buf->f_blocks  = ds_sb->block_count;          /* total data blocks */
	buf->f_bfree   = ds_sb->free_blocks_count;    /* total of free blocks */
	buf->f_bavail  = ds_sb->free_blocks_count;    /* total of available blocks to
	                                               * unprivileged user.
                                                       * The same as f_bfree since we don't implement
                                                       * block reservation */
	buf->f_files   = ds_sb->inodes_count;         /* total file nodes in filesystem */
	buf->f_ffree   = ds_sb->free_inodes_count;    /* free  file nodes in filesystem */
	buf->f_fsid    = (__kernel_fsid_t) { 0 };     /* "nobody knows what f_fsid is supposed to contain" */
	buf->f_namelen = ds_get_max_filename_len(sb); /* maximum length of filenames */
...
static int ds_fill_super(struct super_block *sb, void *data, int silent)
{ // TODO: recuperer des meilleurs infos: http://lxr.free-electrons.com/source/fs/ext2/super.c#L785
    ...
    root_inode = ds_get_inode(sb, ds_ROOT_INODE);
...
static void ds_put_super(struct super_block *sb)
{
    struct ftfs_fs_info *fsinfo = (struct ftfs_fs_info*)sb->s_fs_info;
    size_t block_group_count;
    int i;
...
static struct dentry *ds_mount(struct file_system_type *fs_type, int flags, const char *dev_name, void *data)
{
    return mount_bdev(fs_type, flags, dev_name, data, ds_fill_super);
}

struct file_system_type ds_type = {
    .owner    = THIS_MODULE,
    .name     = "DeadStartFS",
    .mount    = ds_mount,
...
const struct super_operations ds_ops = {
    .statfs       = ds_statfs,
    .write_inode  = ds_write_inode,
    .evict_inode  = ds_evict_inode,
    .show_options = NULL,
    .put_super    = ds_put_super,
};
